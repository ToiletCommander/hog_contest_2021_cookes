...
def playOnce(turnNum, dice, currentScore, opponentScore, numToRoll, lastTimeTrot, targetScore):
    """simulate one time playing
    returns (newScore, newOpponentScore, canPlayMoreTurn, isMoreBoar, isTimeTrot)
    """

    scoreIncrease = gamecalc.take_turn(numToRoll,opponentScore,dice,targetScore)
    isMoreBoar = gamecalc.more_boar(currentScore+scoreIncrease,opponentScore)
    isTimeTrot = gamecalc.time_trot(turnNum,numToRoll,lastTimeTrot) if not(isMoreBoar) else False
    return (currentScore+scoreIncrease,opponentScore,isMoreBoar or isTimeTrot, isMoreBoar, isTimeTrot)
...
def getWinningChance(currentPlayerLastTimeTrot, opponentLastTimeTrot, numToRoll, selfScore, opponentScore, targetScore, turnNum = 0, currentPlayer = 0):
    ...
    playReturn = playOnce(turnNum,dice,selfScore,opponentScore,numToRoll,currentPlayerLastTimeTrot,targetScore)

    newScore, newOpponentScore, canPlayMoreTurn, usedBoar, usedTimeTrot = playReturn
    ...
    if canPlayMoreTurn:
        ...
        for i in range(0,numChance+1):
            sumChance += getWinningChance(usedTimeTrot,opponentLastTimeTrot,i,newScore,newOpponentScore,targetScore,turnNum+1,0)
        ...
        numChance = 10
        for i in range(0,numChance+1):
            sumChance += getWinningChance(usedTimeTrot,opponentLastTimeTrot,i,newScore,newOpponentScore,targetScore,turnNum+1,1)
        chance = sumChance / numChance
        getWinningChance.result_dict[currentKey] = chance
        return chance
